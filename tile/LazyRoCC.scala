...
class RoCCIO(val nPTWPorts: Int, nRoCCCSRs: Int)(implicit p: Parameters) extends RoCCCoreIO(nRoCCCSRs)(p) {
  ...
  //val din = if(tileParams.tileId == 1 || tileParams.tileId == 2 || tileParams.tileId == 3) Some(Flipped(Decoupled(UInt(160.W)))) else None
  //val rocc_in = if(tileParams.tileId == 1 || tileParams.tileId == 2 || tileParams.tileId == 3) Some(Flipped(Decoupled(UInt(55.W)))) else None
  ...
  val fifo_io = if(tileParams.tileId == 0) Some(Vec(10, Decoupled(UInt(64.W)))) else None
